...
def _decode_ascii85(blob: str) -> bytes:
    try:
        return base64.a85decode(blob.encode("ascii"), adobe=False, ignorechars="\r\n\t ")
    except Exception as exc:  # pragma: no cover - defensive
...
def decode_chunk(chunk: object, script_key: str, *, alphabet: Optional[str] = None) -> tuple[bytes, str]:
...
def main() -> None:
    ...
    decoded, strategy = decode_chunk(chunk, args.script_key, alphabet=alphabet)

    text, notes = _attempt_transforms(decoded)
    if text is None:
    ...
    print(
        f"Decoded chunk {index} -> {args.output} ({len(decoded)} bytes, UTF-8 text via {strategy})")
