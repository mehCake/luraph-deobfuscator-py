...
_COMPLEX_PREFIX_HINTS: Tuple[str, ...] = ("complex_obfuscated",)
_COMPLEX_BANNED_RE = re.compile(
    r"(BXOR|BNOT|dispatcher|opcode_map|vm_step|loadstring|string\.char\s*\()",
    re.IGNORECASE,
)
_COMPLEX_BANNER_RE = re.compile(
    r"^\s*--+\s*(?:example|bootstrap|payload|chunk)",
    re.IGNORECASE,
)
...
def _should_clean_complex(prefix: Optional[str]) -> bool:
    ...
    return any(hint in lowered for hint in _COMPLEX_PREFIX_HINTS)


def _strip_complex_markers(prefix: Optional[str], text: str) -> str:
    if not text or not _should_clean_complex(prefix):
        return text

    cleaned = _COMPLEX_BLOCK_COMMENT_RE.sub("", text)
    lines: List[str] = []
    for line in cleaned.splitlines():
        ...
        if not stripped:
            lines.append(line)
            continue
        if _COMPLEX_BANNER_RE.match(stripped):
            continue
        if _COMPLEX_BANNED_RE.search(line):
            continue
        lines.append(line)
    return "\n".join(lines)


def _collect_chunk_previews(
    ctx: "Context",
    prefix: Optional[str],
    *,
    newline: str,
) -> List[str]:
    if not _should_clean_complex(prefix):
        return []

    decoded = getattr(ctx, "decoded_payloads", None)
    if not decoded:
        return []
    ...
    for entry in decoded:
        if not isinstance(entry, str):
            continue
        candidate = utils.strip_non_printable(entry)
        if not candidate:
            continue
        candidate = normalise_lua_newlines(candidate, newline=newline)
        candidate = _strip_complex_markers(prefix, candidate)
        ...
        if lowered in seen:
            continue
        if "function placeholder_" in lowered:
            continue
        seen.add(lowered)
...
def _ensure_lua_segments(
    ...
    *,
    ...
    newline: str,
    extra_comments: Optional[Iterable[str]] = None,
) -> str:
    ...
    if segments:
        include_chunks = not any(
            "function" in segment.lower() and "placeholder_" not in segment.lower()
    ...
    if include_chunks:
        for preview in _collect_chunk_previews(ctx, prefix, newline=newline):
            if preview not in segments:
                segments.insert(0, preview)

    has_function = any("function" in segment.lower() for segment in segments)
    if not has_function:
        placeholder_name = prefix
        if not placeholder_name and chunk_index is not None:
            placeholder_name = f"chunk_{chunk_index + 1}"
        comments: List[str] = [f"undecoded content (size: {byte_length} bytes)"]
        if chunk_index is not None:
            comments.append(f"chunk index: {chunk_index}")
        if extra_comments:
            for comment in extra_comments:
                if comment:
                    comments.append(str(comment))
        placeholder = lua_placeholder_function(placeholder_name, comments, newline=newline).strip()
        segments.append(placeholder)

    combined = "\n\n".join(segment.strip() for segment in segments if segment.strip())
    if not combined:
        return lua_placeholder_function(
            prefix or "chunk",
            [f"undecoded content (size: {byte_length} bytes)"],
            newline=newline,
        )
    return normalise_lua_newlines(combined + "\n", newline=newline)


def _prepare_lua_artifact(
    ctx: "Context",
    raw_text: str,
    *,
    prefix: Optional[str],
    ...
    if sanitized:
        sanitized = normalise_lua_newlines(sanitized, newline=newline)
    else:
        sanitized = ""
    sanitized = _strip_complex_markers(prefix, sanitized)
    sanitized = normalise_lua_newlines(sanitized, newline=newline)
    return _ensure_lua_segments(
        ctx,
        prefix,
        sanitized,
...
def _persist_decoded_outputs(
    ...
    *,
    ...
    newline = "\r\n" if "\r\n" in raw_text else "\n"
    sanitized = _prepare_lua_artifact(
        ctx,
        raw_text,
        prefix=f"chunk_{chunk_index + 1}",
        byte_length=byte_length,
        chunk_index=chunk_index,
        newline=newline,
        extra_comments=["no decoded Lua emitted by initv4 fallback"],
    )
...
def _ensure_bootstrap_blob_placeholder(source: Optional[str | Path]) -> Optional[str]:
    """Ensure the canonical bootstrap blob artefact exists for the CLI."""
    ...
    if isinstance(source, (bytes, bytearray)):
        ...
        try:
            ...
            if source_path.exists() and source_path.is_file():
                ...
                raw_bytes = source.encode("utf-8", errors="ignore")
    ...
    encoded = base64.b64encode(raw_bytes).decode("ascii") if raw_bytes else ""
    if utils.safe_write_file(str(target), encoded):
        return str(target)
    return None
...
def _persist_chunk_outputs(ctx: "Context", text: str) -> List[str]:
    ...
    sanitized = _prepare_lua_artifact(
        ...
        byte_length=byte_length,
        newline=newline,
        extra_comments=["empty pipeline output"],
    )
...
def _iterative_initv4_decode(
    ...
    *,
    ...
    extracted_key = getattr(decoder, "extracted_script_key", None)
    adopted_from_bootstrap = False
    if (not script_key_value or not script_key_value.strip()) and isinstance(
        extracted_key, str
    ):
        cleaned_key = extracted_key.strip()
        if cleaned_key:
            script_key_value = cleaned_key
            adopted_from_bootstrap = True
            ctx_proxy.script_key = cleaned_key
            if not isinstance(getattr(ctx, "script_key", None), str) or not getattr(
                ctx, "script_key"
            ):
                try:
                    ctx.script_key = cleaned_key
                except Exception:  # pragma: no cover - defensive
                    pass
            options = getattr(ctx, "options", None)
            if isinstance(options, dict) and not options.get("script_key"):
                options["script_key"] = cleaned_key
            report_obj = getattr(ctx, "report", None)
            if report_obj is not None and not getattr(report_obj, "script_key_used", None):
                try:
                    report_obj.script_key_used = cleaned_key
                except Exception:  # pragma: no cover - defensive
                    pass

    if not script_key_value:
        return [], source, {}

    try:
        ctx_proxy.script_key = script_key_value
    except Exception:  # pragma: no cover - defensive
        pass
    ...
    masked_key = _mask_script_key(script_key_value)
    ...
    if script_key_value:
        ...
        if adopted_from_bootstrap:
            payload_meta["script_key_provider"] = "bootstrap"
    warnings: List[str] = []
    decoded_chunks: List[Dict[str, Any]] = []
    current_text = source
    key_bytes = script_key_value.encode("utf-8")
...
def run(ctx: "Context") -> Dict[str, Any]:
    ...
    if isinstance(script_key, str):
        ...
        if not script_key:
            script_key = None
    if isinstance(script_key, str) and script_key:
        try:
            ctx.script_key = script_key
        except Exception:  # pragma: no cover - defensive
            pass
        report_obj = getattr(ctx, "report", None)
        if report_obj is not None and not getattr(report_obj, "script_key_used", None):
            try:
                report_obj.script_key_used = script_key
            except Exception:  # pragma: no cover - defensive
                pass
    ...
    if isinstance(version.name, str) and version.name.startswith("luraph_v14_4"):
        ...
        if chunk_meta:
            ...
            payload_meta_hint = chunk_meta.get("handler_payload_meta")
            if (not isinstance(script_key, str) or not script_key) and isinstance(
                payload_meta_hint, Mapping
            ):
                auto_key = payload_meta_hint.get("script_key")
                if isinstance(auto_key, str) and auto_key.strip():
                    script_key = auto_key.strip()
                    try:
                        ctx.script_key = script_key
                    except Exception:  # pragma: no cover - defensive
                        pass
                    options = getattr(ctx, "options", None)
                    if isinstance(options, dict) and not options.get("script_key"):
                        options["script_key"] = script_key
                    report_obj = getattr(ctx, "report", None)
                    if report_obj is not None and not getattr(
                        report_obj, "script_key_used", None
                    ):
                        try:
                            report_obj.script_key_used = script_key
                        except Exception:  # pragma: no cover - defensive
