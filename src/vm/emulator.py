...
class LuraphVM:
    """Stack-based virtual machine used during devirtualization.

    The emulator executes instructions defined by ``bytecode`` using
    functions from :mod:`vm.opcodes`. The design keeps state and opcode
    handlers separate so symbolic execution can hook into the process
    ...
    def load_bytecode(self, payload: Any) -> None:
        ...
        if isinstance(payload, str):
            ...
            raise VMEmulationError("invalid bytecode payload")

        constants = data.get("constants")
        bytecode = data.get("bytecode") or data.get("code")
