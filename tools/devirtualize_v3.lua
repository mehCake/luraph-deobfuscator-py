...
local function find_unpacked()
  local cand = rawget(_G, 'unpackedData')
  if cand ~= nil then
    return cand
  end
  if type(_ENV) == 'table' then
    local env_val = rawget(_ENV, 'unpackedData')
    if env_val ~= nil then
      return env_val
    end
    local env_alt = rawget(_ENV, 'unpacked')
    if env_alt ~= nil then
      return env_alt
    end
    local env_global = rawget(_ENV, '_G')
    if type(env_global) == 'table' then
      local nested = rawget(env_global, 'unpackedData')
      if nested ~= nil then
        return nested
      end
      local nested_alt = rawget(env_global, 'unpacked')
      if nested_alt ~= nil then
        return nested_alt
      end
    end
  end
  local alt = rawget(_G, 'unpacked')
  if alt ~= nil then
    return alt
  end
  return nil
